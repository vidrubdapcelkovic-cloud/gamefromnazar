...
class GameScene extends Phaser.Scene {
  ...
  unregisterChunkWorldObject(id) {
    ...
    if (runtimeObject.blockerObject) {
      const group = this.blockingWorldObjects;
      // Phaser.GameObjects.Group#destroy sets children=undefined. During GameScene SHUTDOWN
      // the StaticPhysicsGroup is destroyed before ChunkManager cleanup calls contains().
      if (group && group.children && group.contains(runtimeObject.blockerObject)) {
        group.remove(runtimeObject.blockerObject, true, true);
  ...
  update(time, delta) {
    ...
    // River water is passable but slows the player. Recomputed every frame from
    // the base speed (never accumulates); water is decided by the feet, i.e. the
    // bottom-centre of the physics body, via the production RiverGenerator mask.
    const foot = PlayerWaterState.footPosition(this.player.body);
    if (foot) {
      const waterMultiplier = PlayerWaterState.speedMultiplier(this.worldSeed, foot.x, foot.y);
      if (waterMultiplier !== 1) movement.scale(waterMultiplier);
    }
