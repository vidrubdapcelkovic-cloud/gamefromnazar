...
// Build-time asset embedding: read the compact production PIG PNG and emit a
// deterministic JS module exposing it as a data URL. This keeps the single
// offline HTML self-contained (no external PNG) and avoids hand-written base64.
const PIG_SOURCE_RELATIVE_PATH = 'assets/generated/pig.png';
const PIG_TEXTURE_MODULE_RELATIVE_PATH = 'src/generated/PigTextureData.js';

function generatePigTextureModule() {
  const sourcePath = path.join(root, PIG_SOURCE_RELATIVE_PATH);
  if (!fs.existsSync(sourcePath) || !fs.statSync(sourcePath).isFile()) {
    throw new Error(`Не найден production-ассет PIG: ${PIG_SOURCE_RELATIVE_PATH}`);
  }
  const base64 = fs.readFileSync(sourcePath).toString('base64');
  const dataUrl = `data:image/png;base64,${base64}`;
  const modulePath = path.join(root, PIG_TEXTURE_MODULE_RELATIVE_PATH);
  ensureDirectory(path.dirname(modulePath));
  const contents = `// AUTO-GENERATED by build.js from ${PIG_SOURCE_RELATIVE_PATH}. Do not edit by hand.\n`
    + '// Build-time embedded PIG texture as a data URL (keeps the offline HTML self-contained).\n'
    + `const PIG_TEXTURE_DATA_URL = '${dataUrl}';\n`;
  return writeFileWithSize(modulePath, contents);
}

const pigTextureModuleSize = generatePigTextureModule();
...
console.log(`PIG texture (встроен): ${PIG_TEXTURE_MODULE_RELATIVE_PATH} (${pigTextureModuleSize} байт)`);
