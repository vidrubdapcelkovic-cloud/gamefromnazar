...
// Build-time asset embedding: read compact production PNGs and emit deterministic
// JS modules exposing them as data URLs. Keeps the single offline HTML self-contained
// (no external PNG) and avoids hand-written base64 next to NPC logic.
function generateEmbeddedTextureModule(sourceRelativePath, moduleRelativePath, constName, label) {
  const sourcePath = path.join(root, sourceRelativePath);
  if (!fs.existsSync(sourcePath) || !fs.statSync(sourcePath).isFile()) {
    throw new Error(`Не найден production-ассет ${label}: ${sourceRelativePath}`);
  }
  const base64 = fs.readFileSync(sourcePath).toString('base64');
  const dataUrl = `data:image/png;base64,${base64}`;
  const modulePath = path.join(root, moduleRelativePath);
  ensureDirectory(path.dirname(modulePath));
  const contents = `// AUTO-GENERATED by build.js from ${sourceRelativePath}. Do not edit by hand.\n`
    + `// Build-time embedded ${label} texture as a data URL (keeps the offline HTML self-contained).\n`
    + `const ${constName} = '${dataUrl}';\n`;
  return writeFileWithSize(modulePath, contents);
}

const pigTextureModuleSize = generateEmbeddedTextureModule(
  'assets/generated/pig.png',
  'src/generated/PigTextureData.js',
  'PIG_TEXTURE_DATA_URL',
  'PIG'
);
const llamaTextureModuleSize = generateEmbeddedTextureModule(
  'assets/generated/llama.png',
  'src/generated/LlamaTextureData.js',
...
const bowmanTextureModuleSize = generateEmbeddedTextureModule(
  'assets/generated/bowman.png',
  'src/generated/BowmanTextureData.js',
...
const playerWaterState = fs.readFileSync(requireFile('src/world/PlayerWaterState.js'), 'utf8');
...
const riverDebugLocator = fs.readFileSync(requireFile('src/world/RiverDebugLocator.js'), 'utf8');
...
${safeScript(playerWaterState)}
${safeScript(villageGenerator)}
${safeScript(chunkGenerator)}
${safeScript(riverDebugLocator)}
...
<html lang="ru">
  ...
  <body>
    ...
    <script src="./src/world/PlayerWaterState.js"></script>
    <script src="./src/world/VillageGenerator.js"></script>
    <script src="./src/world/ChunkGenerator.js"></script>
    <script src="./src/world/RiverDebugLocator.js"></script>
...
const docsGeneratedDirectory = path.join(docsSrcDirectory, 'generated');
...
[docsDirectory, docsLibDirectory, docsSrcDirectory, docsDataDirectory, docsGeneratedDirectory, docsWorldDirectory, docsSystemsDirectory, docsUiDirectory, docsControllersDirectory, docsStylesDirectory, ...assetDirectories]
...
fs.copyFileSync(requireFile('src/world/PlayerWaterState.js'), path.join(docsWorldDirectory, 'PlayerWaterState.js'));
...
fs.copyFileSync(requireFile('src/world/RiverDebugLocator.js'), path.join(docsWorldDirectory, 'RiverDebugLocator.js'));
