/**
 * Deterministic preparation of assets/generated/buffalo.png.
 *
 * Input is typically assets/generated/buivol.png (RGB greenscreen) or an already
 * prepared RGBA buffalo.png. This script:
 * 1. Decodes RGB or RGBA into memory.
 * 2. Samples the border to estimate the studio background key colour.
 * 3. Flood-fills background from the outer perimeter to alpha=0.
 * 4. Applies a minimal local fringe decontamination on silhouette edges.
 * 5. Crops to the opaque content bbox + 16 px transparent padding (no resize).
 * 6. Writes RGBA PNG to assets/generated/buffalo.png via a temp file.
 *
...
const SOURCE_RELATIVE = 'assets/generated/buivol.png';
const TARGET_RELATIVE = 'assets/generated/buffalo.png';
...
function resolveInputPath() {
  ...
  if (fs.existsSync(SOURCE_PATH)) return SOURCE_PATH;
  throw new Error(`Missing ${SOURCE_RELATIVE} or ${TARGET_RELATIVE}`);
}

function prepareBuffaloAsset() {
  ...
  const tempPath = path.join(
    ...
    `.buffalo-prep-tmp-${process.pid}-${Date.now()}.png`
  );
...
if (require.main === module) {
  const result = prepareBuffaloAsset();
  console.log('prepare-buffalo-asset: ok');
